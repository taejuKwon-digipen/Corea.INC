...
FieldPart::FieldPart(FieldHero& hero) :
	hero(hero), stageNum(stageNum), title(), GOM(), mainGame(InputKey::Keyboard::Escape), instNext(InputKey::Keyboard::Enter), showInstruction(true), showIns(0)
{
	stageNum = 1;
	npc = nullptr;
	workerptr = nullptr;
...
void FieldPart::Load()
{
	...
	button->Add(math::ivec2((double)Engine::GetWindow().GetSize().x * (double)0.77, (double)Engine::GetWindow().GetSize().y * 0.66),
		"assets/FieldPart/Button/NB1.png", "assets/FieldPart/Button/NB0.png", false);
	button->Add(math::ivec2((double)Engine::GetWindow().GetSize().x * (double)0.13, (double)Engine::GetWindow().GetSize().y * 0.13),
		"assets/FieldPart/Button/F1.png", "assets/FieldPart/Button/F0.png", false);
	button->Add(math::ivec2((double)Engine::GetWindow().GetSize().x * (double)0.77, (double)Engine::GetWindow().GetSize().y * 0.13),
...
void FieldPart::Draw()
{
	...
	boredTexture.Draw(math::TranslateMatrix(math::ivec2{ 0, Engine::GetWindow().GetSize().y /2 - 100}));
	douptTexture.Draw(math::TranslateMatrix(math::ivec2{ 0, Engine::GetWindow().GetSize().y / 2  }));
	tiredTexture.Draw(math::TranslateMatrix(math::ivec2{ 0, Engine::GetWindow().GetSize().y / 2 + 100 }));
	timerTexture.Draw(math::TranslateMatrix(math::ivec2{ 10, Engine::GetWindow().GetSize().y - 100}));

	math::TransformMatrix data{};
	GOM->DrawAll(data);
...
{
	vec2 HeroPos = {700,400};
	vec2 PlayerPos = npc->endpos;
	...
	vec2 direction = vec2{400, 0};
	vec2 A2 = { PlayerPos.x - HeroPos.x, PlayerPos.y - HeroPos.y };
