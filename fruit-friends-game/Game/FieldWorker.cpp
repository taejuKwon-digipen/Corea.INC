...
FieldWorker::FieldWorker(math::vec2 startPos) : Downkey(InputKey::Keyboard::Down),
GameObject(workerpos), workerpos(startPos){
	draw_worker = false;
	AddGOComponent(new Sprite("assets/FieldPart/FieldWorker.spt", this));
	GetGOComponent<Sprite>()->PlayAnimation(static_cast<int>(FieldWorker_Anim::None_Anim));
	currState = &statenone;
...
void FieldWorker::state_none::Enter(GameObject* object)
{
	...
	worker->GetGOComponent<Sprite>()->PlayAnimation(static_cast<int>(FieldWorker_Anim::None_Anim));
...
void FieldWorker::state_down::Enter(GameObject* object)
{
	FieldWorker* worker = static_cast<FieldWorker*>(object);
	worker->return_type = 2;
	worker->GetGOComponent<Sprite>()->PlayAnimation(static_cast<int>(FieldWorker_Anim::Down_Anim));
}
...
void FieldWorker::Draw(math::TransformMatrix displayMatrix)
{
	if (draw_worker == true)
	{
