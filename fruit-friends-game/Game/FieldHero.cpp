...
FieldHero::FieldHero(math::vec2 startPos) : //add key for moving abd hero position vector
	GameObject(heroPos),
	heroPos(startPos),
	drawHero(true),
	heart(3)
{
	AddGOComponent(new Sprite("assets/FieldPart/Heroine/FieldHeroine.spt", this));
	currState = &Dot;
...
int FieldHero::check_status(GameObject* object)
{
	...
	else if (hero->currState == &hero->Dot)
	{
		return 1;
	}
...
void FieldHero::Draw(math::TransformMatrix displayMatrix)
{
	if (drawHero == true)
	{
...
void FieldHero::state_none::Enter(GameObject* object) {
	...
	hero->GetGOComponent<Sprite>()->PlayAnimation(static_cast<int>(FieldHero_Anim::None_Anim));
}
...
void FieldHero::state_Dot::Enter(GameObject* object) {
	FieldHero* hero = static_cast<FieldHero*>(object);
	hero->check_status(hero);
	hero->GetGOComponent<Sprite>()->PlayAnimation(static_cast<int>(FieldHero_Anim::Walk_Anim));
}
