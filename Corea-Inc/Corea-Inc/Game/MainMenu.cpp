...
void MainMenu::Load() {
	...
	buttonPtr->Add(glm::ivec2(Engine::GetWindow().GetSize().x / 2 - 70, Engine::GetWindow().GetSize().y * 0.28),
		"assets/start0Button.png", "assets/start1Button.png", false);
	buttonPtr->Add(glm::ivec2(Engine::GetWindow().GetSize().x / 2 - 70, Engine::GetWindow().GetSize().y * 0.15),
...
void MainMenu::Draw() {
	...
	{
		//cutSceneImg.drawImage()
		cutSceneImg.init(texX, 0, 1440, 810);
		cutSceneImg.drawImage();
	}
	else {
